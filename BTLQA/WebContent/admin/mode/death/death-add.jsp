...
<body>
	...
	<h2>Thêm mới chế độ tử BHXH</h2>
	<div>
		<form action="<c:url value= '/admin/mode/death/add'/>" method="post"
			id="form" id="form">
			<div class="table-add">
				<table>
					<tr>
						...
						<th><input class="input-add" type="text" name="baseSalary"
							required id="baseSalary" /></th>
					</tr>
					<tr>
						<td><label>Hệ số</label></td>
						<th><input class="input-add" type="number" name="coefficient"
							min=0 max=10 step=".1" required
							onkeypress="return inputDouble(event)" id="coefficient" /></th>
					</tr>
					<tr>
						<td><label>Số năm đóng tối thiểu</label></td>
						<th><input class="input-add" type="number" name="year"
							required min=1 max=50 onkeypress="return inputNumber(event)"
							id="year" /></th>
					</tr>
				</table>
				<table>
