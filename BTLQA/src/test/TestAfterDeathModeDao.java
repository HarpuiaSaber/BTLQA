...
//	private Double reduction;
	...
	public void insert() {
		...
		try {
			...
			Assert.assertNotNull(afterDeathMode);
			Assert.assertTrue(afterDeathMode.getId() > 14);
			Assert.assertEquals(7, afterDeathModeDao.getAll().size());
	...
	public void update() {
		...
		afterDeathMode.setReduction(0.25);
		
		try {
			connection.setAutoCommit(false);
			afterDeathModeDao.update(afterDeathMode);
			
			Assert.assertEquals(6, afterDeathModeDao.getAll().size());
