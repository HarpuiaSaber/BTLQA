...
import service.impl.UserGettingHistoryServiceImpl;
import utils.StringUtils;

@WebServlet(urlPatterns = "/admin/insurance/getting-history-list")
public class GettingHistoryController extends HttpServlet {

	private static final long serialVersionUID = 1L;
	private UserGettingHistoryService ughService = new UserGettingHistoryServiceImpl();

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		req.setCharacterEncoding("UTF-8");
		HttpSession httpSession = req.getSession();
		Object object = httpSession.getAttribute("searchGH");
		Search search = new Search();
		if (object != null) {
			search = (Search) object;
		} else {
			httpSession.setAttribute("searchGH", search);
		}
		...
		req.setAttribute("histories", ughService.searchWithPaging(search, paging));
		req.setAttribute("totalRecord", ughService.getTotalRecord(search));
		RequestDispatcher dispatcher = req.getRequestDispatcher("/admin/insurance/getting-history.jsp");
